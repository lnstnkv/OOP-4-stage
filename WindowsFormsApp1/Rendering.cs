            ...
            List<Fruit> fruits, List<FruitingPlant> fruitingPlants, List<Human> humans, bool isSeason,int mastingSize)
        {
            ...
            foreach (var animal in animals)
            {
                ...
                if (animal is HerbivoresAnimal)
                {
                    ...
                    {
                        case Rabbit _:
                            graph.DrawImage(Rabbit, new Rectangle(position.X * mastingSize, position.Y * mastingSize, mastingSize, mastingSize));
                            break;
                        case Horse _:
                            graph.DrawImage(Horse, new Rectangle(position.X * mastingSize, position.Y * mastingSize, mastingSize, mastingSize));
                            break;
                        case Elephant _:
                            graph.DrawImage(Elephant, new Rectangle(position.X * mastingSize, position.Y * mastingSize, mastingSize, mastingSize));
                            break;
                    }
                }
                ...
                if (animal is OmnivoresAnimal)
                {
                    ...
                    {
                        case Mouse _:
                            graph.DrawImage(Mouse, new Rectangle(position.X * mastingSize, position.Y * mastingSize, mastingSize, mastingSize));
                            break;
                        case Squirrel _:
                            graph.DrawImage(Squirrel, new Rectangle(position.X * mastingSize, position.Y * mastingSize, mastingSize, mastingSize));
                            break;
                        case Pig _:
                            graph.DrawImage(Pig,
                ...
                if (animal is CarnivoresAnimal)
                {
                    ...
                    {
                        case Owl _:
                            graph.DrawImage(Owl, new Rectangle(position.X * mastingSize, position.Y * mastingSize, mastingSize, mastingSize));
                            break;
                        case Lynx _:
                            graph.DrawImage(Lynx, new Rectangle(position.X * mastingSize, position.Y * mastingSize, mastingSize, mastingSize));
                            break;
                        case Eagle _:
                            graph.DrawImage(Eagle, new Rectangle(position.X * mastingSize, position.Y * mastingSize, mastingSize, mastingSize));
                            break;
                    }
                }
                ...
                if (animal is Human)
                {
                    graph.DrawImage(Human, new Rectangle(position.X * mastingSize, position.Y * mastingSize, mastingSize, mastingSize));
                }
            }
            ...
            foreach (var plant in plants)
            {
                ...
                if (plant.IsVirulence() && plant.IsEat()) // ядовитое съедобное 
                    graph.DrawImage(pink, new Rectangle(position.X * mastingSize, position.Y * mastingSize, mastingSize, mastingSize));
                if (!plant.IsVirulence() && plant.IsEat()) // неядовитое съедобное
                    graph.DrawImage(blue, new Rectangle(position.X *mastingSize, position.Y * mastingSize, mastingSize, mastingSize));
                if (plant.IsVirulence() && !plant.IsEat()) // ядовитое несъедобное
                    graph.DrawImage(orange, new Rectangle(position.X *mastingSize, position.Y * mastingSize, mastingSize, mastingSize));
                if (!plant.IsVirulence() && !plant.IsEat()) // неядовитое несъедобное
                    graph.DrawImage(red, new Rectangle(position.X *mastingSize, position.Y * mastingSize, mastingSize, mastingSize));
            }

            foreach (var fruit in fruits)
            {
                var position = fruit.GetPoint();
                graph.FillRectangle(Brushes.DarkBlue, new Rectangle(position.X + 1 * mastingSize, position.Y + 1 * mastingSize, 3, 3));
            }
            ...
            foreach (var fruitingPlant in fruitingPlants)
            {
                ...
                if (fruitingPlant.IsVirulence())
                    graph.DrawImage(virBerry, new Rectangle(position.X *mastingSize, position.Y * mastingSize, mastingSize, mastingSize));
                else
                {
                    graph.DrawImage(berry, new Rectangle(position.X *mastingSize, position.Y * mastingSize, mastingSize, mastingSize));
                }
            }
